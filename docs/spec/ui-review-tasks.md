...
- [x] Extract `DisplaySessionUiState`, `SessionHeroContentKey`, `SessionHeroContentModel`, and
  `resolveSessionHeroContent` into a dedicated `HomeScreenDisplayState.kt` file.
- [x] Extract `HomeOverviewContent` and `resolveHomeOverviewContent` into `HomeOverviewContent.kt`.
- [x] Extract helper composables that are not screen-specific (`ProgressRing`, `StatusBadge`,
...
- [x] Extract `formatDuration()` extension functions and `Instant?.format()` into a shared
...
- [x] Add `import com.alex.lensesreminder.R` and
...
- [x] Compute `displaySession` once in `HomeScreen` and pass it to both `HomeOverviewCard` and
...
- [x] Derive the formatted date with `remember(currentTime) { ... }` keyed on
...
- [x] Remove `localTimeFormatter` and pass the single `timeFormatter` to
...
- [x] Replace all `.value = .value.copy(...)` calls in `SettingsViewModel` with
...
- [x] In `HomeViewModel`, `SettingsViewModel`, and `PlanSessionViewModel`: replace
...
- [x] Add a `DisposableEffect`/`ON_RESUME` observer in `SettingsEditorContent` (or hoist the
...
- [x] Refactor `SettingsEditorScreen` to accept `uiState: SettingsUiState`,
  individual event callbacks (`onMaxWearHoursChanged`, `onSaveClick`, etc.), and
  `snackbarHostState: SnackbarHostState` as parameters.
- [x] Move `collectAsStateWithLifecycle()`, event collection, and ViewModel interaction into
...
- [x] Replace `android.app.DatePickerDialog` usage in `PlanSessionScreen` with Compose Material 3
...
- [x] Migrate `AppDestination` to `@Serializable` data objects (e.g.,
...
- [x] Replace with `pluralStringResource` / `stringResource` using parameterized string
...
- [x] Remove `isLoading = false` from all input-handler `copy()` calls in `SettingsViewModel`.
...
- [x] Consider making `selectedDate` nullable or documenting that the default is always
